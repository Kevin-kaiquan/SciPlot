...
def _open_github_url(request: urllib.request.Request, timeout: int) -> Any:
    _validate_github_https_url(request.full_url)
    response = urllib.request.build_opener(_GitHubHTTPSRedirectHandler()).open(request, timeout=timeout)
    try:
...
def preferred_asset(assets: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    if sys.platform == "darwin":
        ...
        architecture = "arm64" if machine in {"arm64", "aarch64"} else "intel"
...
def download_update(info: UpdateInfo, destination: Path, progress: Callable[[int, str], None]) -> Path:
    ...
    request = urllib.request.Request(info.asset_url, headers={"User-Agent": f"SciPlot/{APP_VERSION}"})
    with _open_github_url(request, timeout=90) as response, temporary.open("wb") as output:
